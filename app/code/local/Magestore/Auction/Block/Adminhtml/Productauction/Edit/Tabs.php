    ...
    protected function _beforeToHtml() {
		...
		if (!$auction || ($auction && ($auction->getStatus() >= 4 ))) { //start
            $this->addTab('form_deposit', array(
                ...
                'class' => 'ajax',
                'url' => $this->getUrl('*/auction_productauction/depositlist', array('id' => $this->getRequest()->getParam('id'))),
            ));
        }
        ...
        if (!$auction || ($auction && ($auction->getStatus() >= 4 ))) { //start
            $this->addTab('form_autobid', array(
                ...
                'class' => 'ajax',
                'url' => $this->getUrl('*/auction_productauction/autobidlist', array('id' => $this->getRequest()->getParam('id'))),
            ));
        }
        ...
        if (!$auction || ($auction && ($auction->getStatus() >= 4 ))) { //start
            $this->addTab('form_watcherlist', array(
                ...
                'class' => 'ajax',
                'url' => $this->getUrl('*/auction_productauction/watcherlist', array('id' => $this->getRequest()->getParam('id'))),
            ));
        }
        ...
        if ($auction && $auction->getStatus() == 5) { //auction end
            $this->addTab('form_winner', array(
                ...
                'class' => 'ajax',
                'url' => $this->getUrl('*/auction_productauction/winnerlist', array('id' => $this->getRequest()->getParam('id'))),
            ));
        }
